...
class _GameplayPageState extends State<GameplayPage> {
  ...
  // Combat entities. The player's own health arrives with the real
  // opponent (multiplayer task); only the target has HP for now.
  late Node dummyNode;
  ...
  void _setupGame() {
    ...
    carBody = RigidBody(
      ...
      mass: 1.0,
      // `angularDamping` is a per-step fraction in [0, 1] and the controller
      // commands the yaw rate itself, so leave it at 0 here.
    );
    ...
    // Collision Listener: damage the training dummy when a bullet hits it.
    physicsWorld.collisions.listen((event) {
      ...
      // Remove the spent projectile so it cannot register extra hits.
  ...
  /// Tints the training dummy so hits are visible on the target itself,
  /// not only in the HUD bar.
  void _refreshDummyAppearance() {
    final t = dummyHealth.percentage; // 1.0 = full, 0.0 = destroyed
    // Grey when healthy, orange in the middle, dark red when destroyed.
  ...
  void _addWall(v.Vector3 pos, v.Vector3 halfExtents) {
    ...
    // A visible fence makes the arena boundary readable (and testable).
    wall.mesh = Mesh(
  ...
  void _resetCar() {
    setState(() {
      // Setting a dynamic body's node transform is treated as a teleport.
        ...
        children: [
          ...
          Positioned(
            ...
            child: JoystickOverlay(
              onChanged: (x, y) {
                // Inputs are read directly by the controller each physics tick,
                // so no setState is needed (and rebuilding the SceneView on
                // every touch move would be wasteful).
